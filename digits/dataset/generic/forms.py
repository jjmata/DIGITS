...
class GenericDatasetForm(DatasetForm):
    ...
    dsopts_feature_encoding = utils.forms.SelectField('Feature Encoding',
            default = 'png',
            choices = [
                ('none', 'None'),
                ('png', 'PNG (lossless)'),
    ...
    dsopts_label_encoding = utils.forms.SelectField('Label Encoding',
            default = 'none',
            choices = [
                ('none', 'None'),
                ('png', 'PNG (lossless)'),
    ...
    dsopts_batch_size = utils.forms.IntegerField('Encoder batch size',
            validators=[
    ...
    dsopts_num_threads = utils.forms.IntegerField('Number of encoder threads',
            validators=[
                validators.DataRequired(),
                validators.NumberRange(min=1),
                ],
    ...
    dsopts_backend = wtforms.SelectField('DB backend',
            choices = [
                ('lmdb', 'LMDB'),
                ],
            default='lmdb',
