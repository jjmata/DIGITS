...
function generateCaffeTree(text){
  window.graph = {nodes: new Array(), links: new Array()};
  ...
  window.layers = _.map(unformattedLayerBlob, function(l,i){
    return {
      ...
      num_outputs: getAttr(l,"num_output:"),
      type: getAttr(l,"type:"),
      tops: getTops(l),
      def: l.join("\n")
    }
  ...
  _.each(window.layers, function(l, index){
      if (l.type == "ReLU" || l.type == "Dropout"){
        l.multi = true;
      }

      // Add nodes and edges to graph
      window.graph.nodes.push(_.extend(l, {index: index}));
      if (!_.isEmpty(l.parent) && !_.isUndefined(l.parent)){
        window.graph.links.push({source: l.parent, target: l});
      }

  });

  // Find corresponding layer for each link already generated by couplings:
  _.each(window.couplings, function(c){
      var source = _.filter(layers, function(l){return l.name == c.parent})[0];
      var target = _.filter(layers, function(l){return l.name == c.name})[0];

      if (!_.isUndefined(source) && !_.isUndefined(target)){
        window.graph.links.push({source: source, target: target});
      }
  });

  return {layers: window.layers, tree: new Object()}
}
