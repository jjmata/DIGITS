...
var LayerVisualizations = function(selector,props){
  ...
  self.drawOutputs = function(json){
    var json = !_.isUndefined(json) ? json : {stats: "Error", msg: "Couldn't retrieve outputs"}

    var msg    = _.noop();
    var status = _.noop();

    // If error, display message in panel instead of outputs:
    if (json.stats == "Error"){
      status = "danger";
      msg = json.msg;
    }

    // Update layer and render panel:
    self.layer.stats = json.stats;
    self.panel.render(msg,status);
...
LayerVisualizations.Jobs = function(selector,props){
  ...
  self.load = function(json){
    if (json.framework == "caffe"){
      generateCaffeTree(json.model_def);
      self.layers_with_outputs = _.pluck(_.filter(graph.nodes, function(n){return !_.isUndefined(n.num_outputs)}),"name");
    } else {
